...
if [[ -n "$VM_ID" && -n "$STORAGE_ID" ]]; then
		...
		# Creates directory for user-data snippets for ignition config
		mkdir -p /var/lib/vz/snippets/
		rm -f /var/lib/vz/snippets/flatcar-user-data-$VM_ID 2>/dev/null
		echo "$IGNITION_CONFIG" > /var/lib/vz/snippets/flatcar-user-data-$VM_ID

		# Set the user-data snippet for ignition config
		qm set $VM_ID --cicustom "user=local:snippets/flatcar-user-data-$VM_ID"
